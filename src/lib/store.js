...
export const addFolder = (folder) => {
  ...
  const f = { id: uid(), items: [], created_date: Date.now(), updated_date: Date.now(), ...folder };
...
// wipe all app data for the current account (folders, lists, journal, files…)
// but keep the account, profile and settings intact.
export const resetAllData = () => {
  const keys = ['folders', 'journal', 'events', 'period', 'alarms', 'map_folders', 'files', 'trash', 'files_trash', 'alarm_tracks', 'deleted_log'];
  keys.forEach((k) => { try { localStorage.removeItem(scopedKey(k)); } catch {} });
  if (typeof window !== 'undefined') window.dispatchEvent(new CustomEvent('retrolist:data-changed'));
};

// move checked checklist items to the bottom (below unchecked ones), preserving
// the relative order of everything else.
export const reorderChecklist = (folderId) => {
  const folders = getFolders();
  const folder = folders.find((f) => f.id === folderId);
  if (!folder) return;
  folder.items = (folder.items || [])
    .map((it, i) => ({ it, i, d: it.kind === 'checklist' && it.done ? 1 : 0 }))
...
// reorder items within a folder by index (used by drag-to-reorder in lists)
export const reorderItemsByIndex = (folderId, from, to) => {
...
// reorder child folders within a parent by moving the folder in the full array
// (getChildFolders filters by parent_id in array order, so this reorders them)
export const reorderChildFoldersByIndex = (parentId, from, to) => {
  if (from === to || from < 0 || to < 0) return;
  const all = getFolders();
  const childIds = all.filter((f) => f.parent_id === parentId).map((f) => f.id);
  if (from >= childIds.length || to >= childIds.length) return;
...
// ── my-files trash (photos/files; auto-purged after 15 days) ──
export const getFileTrash = () => read('files_trash', []);
export const saveFileTrash = (t) => write('files_trash', t);
export const trashPhotoByUrl = (url, source) => {
  if (!url) return;
  const t = getFileTrash();
  t.push({ id: uid(), url, type: 'photo', source: source || '', deletedAt: Date.now() });
  saveFileTrash(t);
  deletePhotoByUrl(url);
};
export const trashFileById = (id) => {
  const f = getAllFiles().find((x) => x.id === id);
  if (!f) return;
  const t = getFileTrash();
  t.push({ id: uid(), url: f.url, name: f.name, type: 'file', deletedAt: Date.now() });
  saveFileTrash(t);
  deleteFile(id);
};
export const restoreFileTrash = (trashId) => {
  const t = getFileTrash();
  const item = t.find((x) => x.id === trashId);
  if (!item) return null;
  saveFileTrash(t.filter((x) => x.id !== trashId));
  return item;
};
export const purgeFileTrash = (trashId) => saveFileTrash(getFileTrash().filter((x) => x.id !== trashId));
export const emptyFileTrash = () => saveFileTrash([]);
export const autoPurgeFileTrash = () => {
  const cutoff = Date.now() - TRASH_TTL;
  const all = getFileTrash();
  const remaining = all.filter((x) => (x.deletedAt || 0) > cutoff);
  if (remaining.length !== all.length) saveFileTrash(remaining);
};
try { autoPurgeFileTrash(); } catch {}