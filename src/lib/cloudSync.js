...
function collectLocalRecords() {
  ...
  if (account) {
    records.push({
      ...
      kind: 'account',
      payload: { username: account.username, salt: account.salt, hash: account.hash, email: account.email || '', email_confirmed: !!account.email_confirmed },
  ...
  switch (kind) {
    ...
    case 'account': {
      if (!isDeleted && payload && payload.username) {
        saveAccount({ username: payload.username, salt: payload.salt, hash: payload.hash, email: payload.email || '', email_confirmed: !!payload.email_confirmed, updated_date: payload.updated_date || Date.now() });
      }
      break;
    }
  }
