...
export default function MediaUploader({ media = [], onChange, allowUrl = false, allowCaption = false, columns = 3, enableViewer = false, selectable = false, onDeleteItems }) {
  ...
  const addFiles = (files) => {
    ...
    if (images.length) {
      Promise.all(images.map((file) => new Promise((res) => {
        const r = new FileReader();
        r.onload = () => res({ file, url: r.result });
        r.readAsDataURL(file);
      }))).then((items) => { setEditQueue(items); setEditIndex(0); });
    }
  };

  const advanceEdited = (addUrl) => {
    const cur = editQueue[editIndex];
    if (addUrl && cur) onChange([...media, { id: uid(), type: 'photo', url: addUrl, name: cur.file.name, size: cur.file.size }]);
    const next = editIndex + 1;
    if (next < editQueue.length) setEditIndex(next);
    else { setEditQueue([]); setEditIndex(0); }
  };
  ...
  return (
    <div className="space-y-3">
      ...
      {editQueue.length > 0 && editQueue[editIndex] && (
        <PhotoGridEditor
          imageSrc={editQueue[editIndex].url}
          index={editIndex}
          total={editQueue.length}
          onAdd={(url) => advanceEdited(url)}
          onSkip={() => advanceEdited()}
          onCancel={() => { setEditQueue([]); setEditIndex(0); }}
        />
      )}
    </div>
  );
