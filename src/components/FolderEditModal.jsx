...
export default function FolderEditModal({ open, onClose, onSave, folder, defaultType, defaultParentId }) {
  ...
  useEffect(() => {
    if (open) {
      const label = LIST_TYPES[folder?.type || defaultType]?.label || 'folder';
      setName(folder?.name || ('new ' + label));
  ...
  useEffect(() => {
    if (!dupToast) return;
    const t = setTimeout(() => setDupToast(false), 2600);
    return () => clearTimeout(t);
  }, [dupToast]);

  if (!open) return null;

  const sz = BLOCK_SIZES.find((s) => s.id === (size || 'portrait')) || BLOCK_SIZES[1];
  const [aw, ah] = sz.ratio.split('/').map((n) => Number(n.trim()));
  const coverAspect = aw / ah;

  const handleSave = () => {
    if (!name.trim()) return;
    const dup = getFolders().some((f) => f.id !== folder?.id && (f.name || '').trim().toLowerCase() === name.trim().toLowerCase());
    if (dup) { setDupToast(true); return; }
    onSave({ name: name.trim(), type, color, cover, emoji: emoji.trim(), size, subtitle: subtitle.trim(), ...(defaultParentId && !folder ? { parent_id: defaultParentId } : {}) });
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" onClick={onClose}>
      ...
      >
        ...
        {dupToast && (
          <div className="mb-3 px-3 py-2 rounded-xl bg-destructive/10 text-destructive text-xs font-medium lowercase text-center animate-fade-in">
            name is in use — rename to continue
          </div>
        )}
        <div className="space-y-4">
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground lowercase block mb-1.5">subheading</label>
            <input
              value={subtitle}
        ...
        <p className="text-[10px] text-muted-foreground/60 lowercase text-center mt-5">
          created {new Date(folder?.created_date || Date.now()).toLocaleString()}
        </p>
