...
export default function FolderPicker({ open, onClose, onSelect, title = 'move to', allowMainPage = true, excludeTypes = [] }) {
  const [folders, setFolders] = useState([]);
  useEffect(() => { if (open) setFolders(getFolders().filter((f) => !excludeTypes.includes(f.type))); }, [open, excludeTypes]);
